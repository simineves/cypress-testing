...
describe("Pre and Post Match Page Tests", () => {
  it("Adding Pre and Post Matches URL in Json file", () => {
    ...
    cy.get(".mls-c-schedule__matches").then((schedule) => {
      ...
      cy.writeFile("NEXTPRO_URLS.json", postMatchObj);
      cy.log(`postMatchURLs array written to: NEXTPRO_URLS.json`);
    });
    ...
    cy.get(".mls-c-schedule__matches").then((schedule) => {
      ...
      cy.writeFile("NEXTPRO_URLS.json", tempUrlsObj);
      cy.log(`preMatchURLs array written to: temp-NEXTPRO_URLS.json`);
    });
  });
});
