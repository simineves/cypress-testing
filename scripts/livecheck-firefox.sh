...
if [ "$type" == "MLS" ]; then
    ...
    npx cypress run --record --key $CYPRESS_MLS_LIVE_KEY --browser firefox --tag 'mls-live-firefox' --spec 'cypress/E2E/MLS/LiveTests/MatchPage' --config-file './cypress.mls-live.config.js'
    ...
    npx cypress run --record --key $CYPRESS_NEXTPRO_LIVE_KEY --browser firefox --tag 'mlsnp-live-firefox' --spec 'cypress/E2E/NEXTPRO/LiveTests/MatchPage' --config-file './cypress.nextpro-live.config.js'
else
